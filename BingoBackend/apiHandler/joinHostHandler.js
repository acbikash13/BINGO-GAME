...
async function hostGame(userId) {
  try {
    ...
    let userName = await database
      .collection("users")
      .findOne({ _id: ObjectId(userId) }, { projection: { firstName: 1 } });
    ...
    // make sure we only push a userId if it does not exist
    if (!gameDocument.bingoBoard.playersJoined.includes(userId)) {
      ...
      if (createGame.acknowledged) {
        ...
        return {
          status: 400,
          message: "Unable to create a game! Please try again!",
        };
      }
    } else {
      return { status: 201, message: "Player is already in the game!" };
    }
  } catch (err) {
    console.error(err);
    return err;
  ...
  try {
    ...
    if (game.length == 1) {
      ...
      if (!game[0].bingoBoard.playersJoined.includes(userId)) {
        ...
        playerStateForThisPlayer.displayName =
          displayNameForThePlayer.firstName;
        playerStateForThisPlayer.userId = userId;
        playerStateForThisPlayer.role = "joinee";
        // insert the player into this game if the player does not exist
        ...
        let updateResult = await collection.updateOne(
          ...
          {
            $push: {
              "bingoBoard.playerStates": playerStateForThisPlayer,
        ...
        if (updateResult.modifiedCount === 0) {
          return {
            status: 400,
            message: "Update field. Please try again to join the game!",
          };
        } else {
          return {
            status: 201,
            message: "Update Successfull. Player joined the game successfully!",
          };
        }
      } else {
        return {
          status: 406,
          message: "The game does not exists, please check your game Id!",
        };
      }
    } else {
      return { status: 201, message: "Player is already in the game!" };
